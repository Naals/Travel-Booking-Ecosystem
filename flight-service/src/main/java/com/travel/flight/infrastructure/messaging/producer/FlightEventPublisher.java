package com.travel.flight.infrastructure.messaging.producer;
...
public class FlightEventPublisher {
    ...
    private String resolveTopic(DomainEvent event) {
        return switch (event.getEventType()) {
            case "FlightScheduled"          -> KafkaTopics.SEARCH_INDEX_FLIGHT;
            case "SeatInventoryUpdated"     -> KafkaTopics.FLIGHT_SEAT_AVAILABILITY_UPDATED;
            case "SeatReserved"             -> KafkaTopics.INVENTORY_RESERVATION_CONFIRMED;
            case "SeatReservationReleased"  -> KafkaTopics.INVENTORY_RESERVATION_RELEASED;
            case "FlightStatusChanged"      -> "flight.status-changed";
            default                         -> null;
        };
    }
}
