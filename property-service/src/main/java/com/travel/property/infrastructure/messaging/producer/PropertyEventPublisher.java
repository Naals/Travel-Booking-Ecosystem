package com.travel.property.infrastructure.messaging.producer;
...
public class PropertyEventPublisher {
    ...
    private String resolveTopic(DomainEvent event) {
        return switch (event.getEventType()) {
            case "PropertyCreated"      -> KafkaTopics.SEARCH_INDEX_PROPERTY;
            case "AvailabilityUpdated"  -> KafkaTopics.PROPERTY_AVAILABILITY_UPDATED;
            case "ReservationPlaced"    -> KafkaTopics.INVENTORY_RESERVATION_CONFIRMED;
            case "ReservationReleased"  -> KafkaTopics.INVENTORY_RESERVATION_RELEASED;
            default                     -> null;
        };
    }
}
