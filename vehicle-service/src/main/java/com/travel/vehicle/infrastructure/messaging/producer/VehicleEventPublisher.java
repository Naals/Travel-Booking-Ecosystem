package com.travel.vehicle.infrastructure.messaging.producer;
...
public class VehicleEventPublisher {
    ...
    private String resolveTopic(DomainEvent event) {
        return switch (event.getEventType()) {
            case "VehicleAddedToFleet"          -> KafkaTopics.SEARCH_INDEX_VEHICLE;
            case "VehicleAvailabilityUpdated"   -> KafkaTopics.VEHICLE_AVAILABILITY_UPDATED;
            case "VehicleReserved"              -> KafkaTopics.INVENTORY_RESERVATION_CONFIRMED;
            case "VehicleReservationReleased"   -> KafkaTopics.INVENTORY_RESERVATION_RELEASED;
            default                             -> null;
        };
    }
}
